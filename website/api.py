...
########################################################## ques double
@api.route('/api/quess_double', methods=['GET', 'POST'])
def api_quess_double():
    id = "1"
    users_ref = db.collection(u'Question_ver2').where(u'Don_Kep', u'==', id).stream()
    detail = []
    deto = {}
    for doc in users_ref:
        de = doc.to_dict()
...
@api.route('/api/ques_double/<id>', methods=['GET', 'POST'])
def api_ques_double_1(id):

	Quiz_detail = db.collection(u'Question_ver2').document(id).get()
	print(Quiz_detail)
	detail = []
	deto = {}
	for doc in Quiz_detail:
